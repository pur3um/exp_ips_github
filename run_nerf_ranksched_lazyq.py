...
try:
    ...
    SingleDeviceSign10RsclFWithAuxAdam = None
from optims.auto_cos_inc_rank import SingleDeviceAutoCosIncWithAuxAdam
from optims.auto_ranksched_lazy_q import SingleDeviceRankSchedLazyQWithAuxAdam
...
def init_results_log_muon(results_path, args, optimizer_muon, optimizer_adam, start):
    ...
    with open(results_path, 'a') as f:
        ...
        f.write("script: run_nerf_ranksched_lazyq.py\n")
        f.write(f"expname: {args.expname}\n")
...
def init_results_log_optim(results_path, args, optimizer, start):
    ...
    with open(results_path, 'a') as f:
        ...
        f.write("train_psnr_log:\n")
        if getattr(args, 'optimizer', '') in {'aux-sign-auto-cos-inc', 'aux-sign-ranksched-lazy-q'}:
            f.write("  lowrank_schedule:\n")
            f.write(f"    rank_start: {args.lowrank_rank_start}\n")
            f.write(f"    rank_end: {args.lowrank_rank_end}\n")
            ...
            f.write(f"    ns_steps: {args.lowrank_ns_steps}\n")
            f.write(f"    scale_mode: {args.lowrank_scale_mode}\n")
            if getattr(args, 'optimizer', '') == 'aux-sign-ranksched-lazy-q':
                f.write("  lazy_q_setup:\n")
                f.write(f"    update_gap: {args.lazy_q_update_gap}\n")
                f.write(f"    rank_refresh_stride: {args.lazy_q_rank_refresh_stride}\n")
                f.write(f"    use_b_ema: {args.lazy_q_use_b_ema}\n")
                f.write(f"    b_ema_decay: {args.lazy_q_b_ema_decay}\n")
                f.write(f"    refresh_on_phase_change: {args.lazy_q_refresh_on_phase_change}\n")
...
def append_results_log_optim(results_path, i, global_step, loss, psnr, optimizer):
    ...
    current_basis_rank = optimizer.param_groups[0].get('current_basis_rank', None)
    current_method = optimizer.param_groups[0].get('current_method', None)
    current_phase = optimizer.param_groups[0].get('current_scheduler_phase', None)
    current_refresh = optimizer.param_groups[0].get('current_lazy_q_refresh_this_step', None)
    current_refresh_reason = optimizer.param_groups[0].get('current_lazy_q_refresh_reason', None)
    rank_log = ""
    if current_rank is not None:
        rank_log += f" muon_rank={current_rank}"
    if current_target_rank is not None:
        rank_log += f" muon_target_rank={current_target_rank}"
    if current_basis_rank is not None:
        rank_log += f" muon_basis_rank={current_basis_rank}"
    if current_method is not None:
        rank_log += f" muon_method={current_method}"
    if current_phase is not None:
        rank_log += f" sched_phase={current_phase}"
    if current_refresh is not None:
        rank_log += f" q_refresh={int(bool(current_refresh))}"
    if current_refresh_reason is not None:
        rank_log += f" q_refresh_reason={current_refresh_reason}"
    with open(results_path, 'a') as f:
        f.write(
            f"iter={i} global_step={global_step} "
            f"loss={loss.item():.10f} psnr={psnr.item():.6f} "
...
def _optimizer_supports_progressive_rank(optimizer_name):
    return optimizer_name in {'aux-sign-auto-cos-inc', 'aux-sign10-rsclF', 'aux-sign-ranksched-lazy-q'}
...
def _sync_optimizer_schedule_config(optimizer, args, reset_rank_fields=False):
    ...
    for group in optimizer.param_groups:
        ...
        if args.optimizer in {'aux-sign-auto-cos-inc', 'aux-sign10-rsclF', 'aux-sign-ranksched-lazy-q'}:
            ...
            if args.optimizer == 'aux-sign-ranksched-lazy-q':
                group['lazy_q_update_gap'] = int(args.lazy_q_update_gap)
                group['lazy_q_rank_refresh_stride'] = int(args.lazy_q_rank_refresh_stride)
                group['lazy_q_use_b_ema'] = bool(args.lazy_q_use_b_ema)
                group['lazy_q_b_ema_decay'] = float(args.lazy_q_b_ema_decay)
                group['lazy_q_refresh_on_phase_change'] = bool(args.lazy_q_refresh_on_phase_change)
            if reset_rank_fields:
                group['current_method'] = (
                    'cosine_phase_lazy_q_auto_start' if (args.optimizer == 'aux-sign-ranksched-lazy-q' and bool(args.lowrank_auto_init_rank_start))
                    else 'cosine_phase_lazy_q_closed_form' if args.optimizer == 'aux-sign-ranksched-lazy-q'
                    else 'cosine_inc_auto_start' if bool(args.lowrank_auto_init_rank_start)
...
def _apply_scheduler_context_to_optimizer(optimizer, phase_name, phase_changed, global_step):
    for param_group in optimizer.param_groups:
        if not param_group.get('use_muon', False):
            continue
        param_group['scheduler_phase_name'] = phase_name
        param_group['scheduler_phase_changed'] = bool(phase_changed)
        param_group['scheduler_global_step'] = int(global_step)
...
def create_nerf(args):
    ...
    if args.optimizer in ('ori-adam', 'adam'):
    ...
    elif args.optimizer == 'aux-sign-ranksched-lazy-q':
        aux_param_groups[0].update(
            dict(
                rank=args.lowrank_rank_start,
                rank_start=args.lowrank_rank_start,
                ...
                lazy_q_rank_refresh_stride=args.lazy_q_rank_refresh_stride,
                lazy_q_use_b_ema=bool(args.lazy_q_use_b_ema),
                lazy_q_b_ema_decay=args.lazy_q_b_ema_decay,
                lazy_q_refresh_on_phase_change=bool(args.lazy_q_refresh_on_phase_change),
            )
        )
        optimizer = SingleDeviceRankSchedLazyQWithAuxAdam(aux_param_groups)
        print(
            f'INFO: Rank-scheduler-aware lazy-Q optimizer configured. Hidden params: {len(muon_params)}, '
            f'Aux params: {len(adam_params)}.'
        )
    elif args.optimizer == 'aux-sign10-rsclF':
        if SingleDeviceSign10RsclFWithAuxAdam is None:
            raise ImportError('optims.lr_sign10_rsclF is unavailable in this environment.')
        aux_param_groups[0].update(
            dict(
                rank=args.lowrank_rank_start,
                rank_start=args.lowrank_rank_start,
        ...
        raise ValueError(
            ...
            "'adam', 'aux-sign', 'aux-sign-auto-cos-inc', 'aux-sign-ranksched-lazy-q', 'aux-sign10-rsclF'."
        )
    #//===================================//#
...
def config_parser():
    ...
    parser.add_argument("--optimizer",
                        ...
                        choices=['adam', 'ori-adam', 'aux-sign', 'aux-sign-auto-cos-inc', 'aux-sign-ranksched-lazy-q', 'aux-sign10-rsclF'],
                        )
    ...
    # Scheduler-aware lazy-Q optimizer options
    parser.add_argument(
        "--lazy_q_update_gap",
        type=int,
        default=100,
        help="Base lazy-Q refresh gap in optimizer steps.",
    )
    parser.add_argument(
        "--lazy_q_rank_refresh_stride",
        type=int,
        default=8,
        help="Force a Q refresh only when target_rank - cached_basis_rank reaches this stride.",
    )
    parser.add_argument(
        "--lazy_q_use_b_ema",
        type=parse_bool_flag,
        default=True,
        help="Whether to apply EMA to B = Q^T X between basis refreshes.",
    )
    parser.add_argument(
        "--lazy_q_b_ema_decay",
        type=float,
        default=0.9,
        help="EMA decay for the projected matrix B = Q^T X.",
    )
    parser.add_argument(
        "--lazy_q_refresh_on_phase_change",
        type=parse_bool_flag,
        default=True,
        help="Force a basis refresh when the outer LR scheduler phase changes.",
    )
...
def train():
    ...
    start = start + 1
    last_scheduler_phase = None if start <= 1 else train_scheduler.step(max(global_step - 1, 0))["phase_name"]
    ...
    for i in trange(start, N_iters):
        ...
        phase_changed = (current_phase != last_scheduler_phase)

        _apply_optimizer_lrs(optimizer, new_muon_lrate, new_adam_lrate)
        _apply_scheduler_context_to_optimizer(optimizer, current_phase, phase_changed, global_step)
        ...
        #// val
        if i % args.i_print==0:
            basis_rank = optimizer.param_groups[0].get('current_basis_rank', None)
            refresh_flag = optimizer.param_groups[0].get('current_lazy_q_refresh_this_step', None)
            refresh_reason = optimizer.param_groups[0].get('current_lazy_q_refresh_reason', None)
            extra_lazy = ""
            if basis_rank is not None:
                extra_lazy += f" basis_rank={basis_rank}"
            if refresh_flag is not None:
                extra_lazy += f" q_refresh={int(bool(refresh_flag))}"
            if refresh_reason is not None:
                extra_lazy += f" q_reason={refresh_reason}"
            tqdm.write(
                f"[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}  "
                f"scheduler={args.train_scheduler} phase={current_phase} N_rand={N_rand} "
                f"lr_ratio={current_lr_ratio:.4f} lr_muon={new_muon_lrate:.3e} lr_adam={new_adam_lrate:.3e}{extra_lazy}"
            )
        ...
        global_step += 1
        last_scheduler_phase = current_phase
