"""Discrete Step Rank Schedule optimizer with optional lazy Q (+ EMA).

Variant 1 of the progressive-rank family:
- keeps the auto-init logic from auto_cos_inc_rank.py
- replaces the smooth cosine rank increase with a piecewise-constant step schedule
- by default preserves the original step-rank behavior (no lazy-Q, no EMA)
- optionally enables lazy-Q refresh and EMA on B = Q^T X

기존 discrete step rank 기본 동작 유지
여기에 선택적으로 lazy Q + B-EMA를 붙일 수 있게 변경
즉, default는 원래 step-rank 그대로이고, flag를 켜면 discrete rank + lazy Q(+EMA)로 동작

Place this file under optims/auto_step_rank.py.
"""
...
def get_discrete_step_rank(
    ...
    stage_idx = min(num_stages - 1, int(math.floor(t * num_stages)))
    stage_progress = stage_idx / float(num_stages - 1)
    rank = start_rank + (end_rank - start_rank) * stage_progress
...
def muon_update(
    ...
    if lazy_q_enabled and state is not None:
...
class SingleDeviceAutoStepRankWithAuxAdam(torch.optim.Optimizer):
    """Muon + auxiliary Adam with discrete stepwise rank growth.

    Defaults preserve the previous step-rank behavior (no lazy Q, no B-EMA).
    To enable the new variant, set lazy_q_enabled=True and optionally lazy_q_use_b_ema=True.
    """
    ...
    def __init__(self, param_groups):
        ...
        for group in param_groups:
            ...
            if g["use_muon"]:
                ...
                g.setdefault("rank_num_stages", 8)
                g.setdefault("lazy_q_enabled", False)
                ...
                g.setdefault("current_method", "step_rank_closed_form")
                g.setdefault("current_lazy_q_gap", None)
                g.setdefault("current_lazy_q_last_refresh_step", None)
                g.setdefault("current_lazy_q_refresh_this_step", None)
                g.setdefault("current_lazy_q_use_b_ema", None)
                g.setdefault("current_lazy_q_b_ema_decay", None)
                g.setdefault("current_lazy_q_b_ema_initialized", None)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            if group["use_muon"]:
                ...
                scheduled_rank = get_discrete_step_rank(
                    step=group_step,
                    start_rank=group["rank_start"],
                    end_rank=group["rank_end"],
                    warmup_steps=group["warmup_steps"],
                    num_stages=group["rank_num_stages"],
                )
                ...
                lazy_q_enabled = bool(group.get("lazy_q_enabled", False))
                group["current_method"] = (
                    "step_rank_lazy_q_auto_start"
                    if (lazy_q_enabled and bool(group.get("auto_init_rank_start", False)))
                    else "step_rank_lazy_q_closed_form"
                    if lazy_q_enabled
                    else "step_rank_auto_start"
                    if bool(group.get("auto_init_rank_start", False))
                    else "step_rank_closed_form"
                )
                if lazy_q_enabled:
                    if bool(group.get("lazy_q_use_b_ema", False)):
                        group["current_method"] += "_b_ema"
                    group["current_lazy_q_gap"] = int(group.get("lazy_q_update_gap", 100))
                    ...
                    group["current_lazy_q_gap"] = None
                    group["current_lazy_q_last_refresh_step"] = None
                    group["current_lazy_q_refresh_this_step"] = None
                    group["current_lazy_q_use_b_ema"] = None
                    group["current_lazy_q_b_ema_decay"] = None
                    group["current_lazy_q_b_ema_initialized"] = None
                ...
                for p in group["params"]:
                    ...
                    update = muon_update(
                        ...
                        lazy_q_enabled=lazy_q_enabled,
                        lazy_q_update_gap=group.get("lazy_q_update_gap", 100),
