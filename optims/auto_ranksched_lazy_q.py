"""Scheduler-aware lazy-Q progressive-rank optimizer.

This variant merges:
- progressive cosine rank growth from auto_cos_inc_rank.py
- lazy Q refresh from auto_lazy_q_update.py
- rank-aware outer scheduler integration from run_nerf_ranksched.py

Key idea:
- LR schedule stays outside the optimizer.
- The runner passes scheduler phase information into the optimizer.
- The optimizer uses a *lagged* basis rank: it does not force a Q refresh on every
  +1 rank increment. Instead it refreshes only when
    (a) the lazy gap expires,
    (b) the scheduler phase changes (e.g. stable -> decay),
    (c) the target rank has moved far enough beyond the cached basis rank.
- Optional EMA over B = Q^T X stabilizes stale-basis steps.
"""
...
@torch.no_grad()
def zeropower_via_scheduler_aware_lazy_lowrank_matrix_sign(
    G: torch.Tensor,
    state: dict,
    *,
    current_step: int,
    ...
    m, n = X.shape
    r = min(m, n)
    target_rank = max(1, min(int(target_rank), r))
    target_sketch_dim = min(target_rank + max(0, oversample), r)

    # Full / near-full path: no low-rank benefit left.
    if target_sketch_dim >= r:
        ...
        state["lazy_q_refresh_reason"] = "full_rank"
        state["lazy_q_applied_rank"] = int(target_rank)
        state["lazy_q_target_rank"] = int(target_rank)
        state["lazy_q_rank"] = int(target_rank)
        state["lazy_q_cache"] = None
        state["lazy_q_b_ema"] = None
        if transposed:
            Z = Z.mT
        return Z.type_as(G)

    refresh_gap = max(1, int(refresh_gap))
    rank_refresh_stride = max(1, int(rank_refresh_stride))
    ...
    cached_rank = int(state.get("lazy_q_rank", 0) or 0)
    last_refresh_step = int(state.get("lazy_q_last_refresh_step", 0))

    rank_delta = max(0, int(target_rank) - int(cached_rank))
    ...
    if cached_Q is None:
        need_refresh = True
        reason = "init"
    elif cached_shape != (m, n) or cached_transposed != transposed:
        need_refresh = True
        reason = "shape"
    elif bool(force_refresh):
        need_refresh = True
        reason = "phase"
    elif rank_delta >= rank_refresh_stride:
        need_refresh = True
        reason = "rank_stride"
    elif (int(current_step) - last_refresh_step) >= refresh_gap:
        need_refresh = True
        reason = "gap"

    if need_refresh:
        applied_rank = target_rank
        applied_sketch_dim = min(applied_rank + max(0, oversample), r)
        Omega = torch.randn(n, applied_sketch_dim, device=X.device, dtype=X.dtype)
        Y = X @ Omega
        Q, _ = torch.linalg.qr(Y, mode="reduced")
        state["lazy_q_cache"] = Q.detach()
        state["lazy_q_shape"] = (m, n)
        state["lazy_q_transposed"] = transposed
        state["lazy_q_rank"] = int(applied_rank)
        state["lazy_q_last_refresh_step"] = int(current_step)
        state["lazy_q_refresh_this_step"] = True
        state["lazy_q_refresh_reason"] = reason
        state["lazy_q_b_ema"] = None  # reset; old basis is incompatible
    else:
        applied_rank = int(cached_rank)
    ...
    if bool(use_b_ema):
        ema = state.get("lazy_q_b_ema", None)
        if ema is None or ema.shape != B.shape:
            ema = B.detach().clone()
        else:
            ema.mul_(float(b_ema_decay)).add_(B, alpha=1.0 - float(b_ema_decay))
        state["lazy_q_b_ema"] = ema
        B_for_ns = ema
    else:
        state["lazy_q_b_ema"] = None
        B_for_ns = B
    ...
    if rescale and Q.shape[1] > 0:
        Z = Z * math.sqrt(float(r) / float(Q.shape[1]))

    if transposed:
        Z = Z.mT

    state["lazy_q_applied_rank"] = int(applied_rank)
    state["lazy_q_target_rank"] = int(target_rank)
    return Z.type_as(G)
...
def muon_update(
    ...
    lazy_q_rank_refresh_stride: int = 8,
    force_refresh: bool = False,
    lazy_q_use_b_ema: bool = True,
    ...
    update = zeropower_via_scheduler_aware_lazy_lowrank_matrix_sign(
        update,
        state,
        current_step=step,
        refresh_gap=lazy_q_update_gap,
        target_rank=applied_rank,
        rank_refresh_stride=lazy_q_rank_refresh_stride,
...
class SingleDeviceRankSchedLazyQWithAuxAdam(torch.optim.Optimizer):
    """Progressive-rank Muon + auxiliary Adam with scheduler-aware lazy Q refresh.

    Intended pairing:
      - rank_wsd scheduler outside the optimizer
      - cosine rank growth inside the optimizer
      - phase-aware lazy Q refresh + optional B-EMA inside the optimizer
    """
    ...
    def __init__(self, param_groups):
        ...
        for group in param_groups:
            ...
            if g["use_muon"]:
                ...
                g.setdefault("lazy_q_rank_refresh_stride", 8)
                g.setdefault("lazy_q_use_b_ema", True)
                g.setdefault("lazy_q_b_ema_decay", 0.9)
                g.setdefault("lazy_q_refresh_on_phase_change", True)
                ...
                g.setdefault("current_basis_rank", g["rank_start"])
                g.setdefault("current_method", "cosine_phase_lazy_q")
                ...
                g.setdefault("current_lazy_q_refresh_reason", "init")
                g.setdefault("current_scheduler_phase", None)
                g.setdefault("current_scheduler_phase_changed", False)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            if group["use_muon"]:
                ...
                target_rank = max(int(group["rank"]), int(scheduled_rank))

                phase_name = group.get("scheduler_phase_name", None)
                phase_changed = bool(group.get("scheduler_phase_changed", False))
                force_refresh = bool(group.get("lazy_q_refresh_on_phase_change", True)) and phase_changed

                group["current_rank"] = target_rank
                group["current_target_rank"] = max(int(group["rank"]), int(group["rank_end"]))
                group["current_method"] = (
                    "cosine_phase_lazy_q_auto_start"
                    if bool(group.get("auto_init_rank_start", False))
                    else "cosine_phase_lazy_q_closed_form"
                )
                group["current_lazy_q_gap"] = int(group["lazy_q_update_gap"])
                group["current_scheduler_phase"] = phase_name
                group["current_scheduler_phase_changed"] = phase_changed
                ...
                for p in group["params"]:
                    ...
                    update = muon_update(
                        ...
                        lazy_q_rank_refresh_stride=group["lazy_q_rank_refresh_stride"],
                        force_refresh=force_refresh,
                        lazy_q_use_b_ema=group["lazy_q_use_b_ema"],
                        lazy_q_b_ema_decay=group["lazy_q_b_ema_decay"],
                    )
                    ...
                    max_basis_rank = max(max_basis_rank, int(state.get("lazy_q_applied_rank", 0)))
                    refresh_reasons.append(str(state.get("lazy_q_refresh_reason", "reuse")))

                    p.mul_(1 - group["lr"] * group["weight_decay"])
                    p.add_(update.reshape(p.shape), alpha=-group["lr"])

                group["current_basis_rank"] = int(max_basis_rank)
                group["current_lazy_q_refresh_this_step"] = refresh_happened
                group["current_lazy_q_last_refresh_step"] = last_refresh_step
                if refresh_reasons:
                    uniq = []
                    for r in refresh_reasons:
                        if r not in uniq:
                            uniq.append(r)
                    group["current_lazy_q_refresh_reason"] = "+".join(uniq)
                else:
                    group["current_lazy_q_refresh_reason"] = "reuse"
            else:
                for p in group["params"]:
                    if p.grad is None:
                        p.grad = torch.zeros_like(p)
