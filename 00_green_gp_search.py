...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=(
            "Scheduler passed to the NeRF runner as --train_scheduler. "
            "'exp_decay' requires the runner to support that choice."
        ),
    )
    ...
    p.add_argument(
        ...
        default=24,
        help=(
            "Total number of Bayesian optimization iterations to reach. "
            "When --resume is used, iterations already present in trials.jsonl are skipped."
        ),
    )
    # Backward-compatible hidden alias. Prefer --num-bo-total in new commands.
    p.add_argument("--num-bo", dest="num_bo_total", type=int, default=argparse.SUPPRESS, help=argparse.SUPPRESS)
    p.add_argument("--q-batch", type=int, default=2, help="Candidates proposed per BO iteration. Default assumes two GPUs.")
    ...
    p.add_argument("--cuda-devices", default="0,1", help="Comma-separated GPU ids. Default assumes two GPUs. Use empty string for CPU/no CUDA_VISIBLE_DEVICES override")
    ...
    for batch in chunks(list(trials), max_parallel):
        with ThreadPoolExecutor(max_workers=len(batch)) as executor:
            ...
            for idx, trial in enumerate(batch):
                ...
                fut = executor.submit(run_one_trial, trial, args, space, cuda, run_basedir, log_dir)
                future_to_trial[fut] = trial
            for fut in as_completed(future_to_trial):
                rec = fut.result()
                # Write trial records only from the main orchestration thread.
                # This avoids concurrent appends to trials.jsonl when multiple
                # GPU workers finish at nearly the same time.
...
BO_PHASE_PATTERN = re.compile(r"^bo_(\d+)$")


def completed_bo_iterations(records: Sequence[Dict[str, Any]]) -> int:
    """Return the number of BO iteration indices already present.

    Phases are named bo_000, bo_001, ...
    If a previous run finished bo_000 and bo_001, this returns 2 so resume
    starts from bo_002. This makes --num-bo-total mean a total target count,
    not an additional count.
    """
    max_idx = -1
    for rec in records:
        phase = str(rec.get("phase", ""))
        match = BO_PHASE_PATTERN.match(phase)
        if match:
...
def main() -> None:
    ...
    print(f"BO iterations target: {args.num_bo_total}")
    print(f"q-batch: {args.q_batch}")
    ...
    start_bo_iter = completed_bo_iterations(records) if args.resume else 0
    if start_bo_iter >= args.num_bo_total:
        print(
            f"Requested --num-bo-total={args.num_bo_total}; "
            f"already have {start_bo_iter} BO iteration(s). No additional BO iterations will run.",
            flush=True,
        )

    for bo_iter in range(start_bo_iter, args.num_bo_total):
        ...
        print(f"\n[BO iter {bo_iter + 1}/{args.num_bo_total}] proposing {len(bo_trials)} trial(s)")
