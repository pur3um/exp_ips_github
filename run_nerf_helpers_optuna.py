...
def ndc_rays(H, W, focal, near, rays_o, rays_d):
    focal = _as_tensor_like(focal, rays_o)
    near = _as_tensor_like(near, rays_o)
...
def sample_pdf(bins, weights, N_samples, det=False, pytest=False):
    ...
    if det:
        u = torch.linspace(0., 1., steps=N_samples, device=device, dtype=dtype)
    ...
    if pytest:
        ...
        if det:
            u_np = np.linspace(0., 1., N_samples)
